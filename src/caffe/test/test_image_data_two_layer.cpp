...
class ImageDataTwoLayerTest : public MultiDeviceTest<TypeParam> {
 public:
  typedef typename TypeParam::Dtype Dtype;

 protected:
      ImageDataTwoLayerTest(): seed_(1701) {}
  
  virtual ~ImageDataTwoLayerTest() {
...
TYPED_TEST_CASE(ImageDataTwoLayerTest, TestDtypesAndDevices);

TYPED_TEST(ImageDataTwoLayerTest, TestSetUp) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  ImageDataTwoLayer<Dtype> layer(param);
  ...
  EXPECT_EQ(this->blob_top_vec_[1]->height(), 256);
  EXPECT_EQ(this->blob_top_vec_[1]->width(), 256);
  EXPECT_EQ(this->blob_top_vec_[2]->count(), 5);
  for(int i=0; i<100; ++i)
    layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // Generating Images
  DumpMatrixToTxt("temp/images1.txt",
                  this->blob_top_vec_[0]->count(),
                  this->blob_top_vec_[0]->cpu_data(),
                  this->blob_top_vec_[0]->shape());
  DumpMatrixToTxt("temp/images2.txt",
