...
class TripletLossLayerTest: public MultiDeviceTest<TypeParam> {
 ...
 protected:
  TripletLossLayerTest()
      : blob_bottom_data_i_(new Blob<Dtype>(5, 3, 5, 5)),
        blob_bottom_data_j_(new Blob<Dtype>(5, 3, 5, 5)),
        blob_bottom_data_k_(new Blob<Dtype>(5, 3, 5, 5)),
...
TYPED_TEST(TripletLossLayerTest,TestForward) {
  ...
  const int dim = this->blob_bottom_data_i_->count() / num;
  Dtype loss(0);
  for (int i = 0; i < num; ++i) {
    Dtype dist_sq_ij(0);
    Dtype dist_sq_ik(0);
    for (int j = 0; j < dim; ++j) {
      Dtype diff_ij = this->blob_bottom_data_i_->cpu_data()[i*dim+j] -
          this->blob_bottom_data_j_->cpu_data()[i*dim+j];
      dist_sq_ij += diff_ij*diff_ij;
      Dtype diff_ik =this->blob_bottom_data_i_->cpu_data()[i*dim+j] -
         this->blob_bottom_data_k_->cpu_data()[i*dim+j];
      dist_sq_ik += diff_ik*diff_ik;
    }
    loss += std::max(Dtype(0.0), margin + dist_sq_ij - dist_sq_ik);
  }
  loss /= Dtype(num) * Dtype(2.);
