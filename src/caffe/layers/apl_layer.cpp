...
void APLLayer<Dtype>::Forward_cpu_v2(const vector<Blob<Dtype>*>& bottom,
  ...
  for(int i=0; i<sums_; ++i){
    for(int j=0; j<M_; ++j){
      caffe_sub(K_, neuron_offset+i*K_, inPlace_memory_.cpu_data()+j*K_,
                temp3_.mutable_cpu_data() + j*K_);
      caffe_cpu_max(K_, temp3_.cpu_data()+j*K_, Dtype(0.), 
                    temp3_.mutable_cpu_data() + j*K_);
      if (i == slope_num){
        caffe_mul(K_, temp3_.cpu_data()+j*K_, slope_last.cpu_data(),
                  temp3_.mutable_cpu_data()+j*K_);
      } else {
        caffe_mul(K_, temp3_.cpu_data()+j*K_, neuron_weight+i*K_,
                  temp3_.mutable_cpu_data()+j*K_);
      }
    }
    caffe_add(count, top_data, temp3_.cpu_data(), top_data);
  }
}
...
void APLLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if(this->layer_param_.apl_param().layer_weight_decay() > 1e-9){
    ...
    if (type == string("Variance")){
      Variance_regularise(slope_num, K_, neuron_weight);
    } else if (type == string("L2")){
      L2_regularise(slope_num*K_, neuron_weight);
    } else {
      LOG(ERROR) << "Unsupported type: " << type 
          << ". Should be one of [\"Variance\", \"L2\"]";
    }
    caffe_add(slope_num*K_, neuron_weight_diff, temp1_.cpu_data(),
              neuron_weight_diff);
    Variance_regularise(sums_, K_, neuron_offset);
    caffe_add(sums_*K_, neuron_offset_diff, temp1_.cpu_data(),
              neuron_offset_diff);
  }
  if (flag) {
...
void APLLayer<Dtype>::Backward_cpu_v2(const vector<Blob<Dtype>*>& top,
  ...
  for (int i=0; i<M_; ++i){
    for (int j=0; j<sums_; ++j){
      ...
      caffe_cpu_large(K_, temp1_.cpu_data()+j*K_, Dtype(0.), 
                      temp1_.mutable_cpu_data()+j*K_);
      if (j==slope_num){
  ...
  for (int i=0; i<M_; ++i){
    caffe_cpu_large(K_, bottom_data+i*K_, Dtype(0.), temp1_.mutable_cpu_data());
    for(int j=0; j<sums_; ++j){
      caffe_sub(K_, neuron_offset+j*K_, bottom_data+i*K_, 
                temp2_.mutable_cpu_data());
      caffe_cpu_large(K_, temp2_.cpu_data(), Dtype(0.), temp2_.mutable_cpu_data());
      if (j == slope_num) {
 ...
 * Param:
 *    weight is a matrix of M*N
 */
template <class Dtype>
void APLLayer<Dtype>::Variance_regularise(const int M, const int N, 
                                          const Dtype* weight){
  int count = M*N;
  caffe_cpu_gemv<Dtype>(CblasNoTrans, M, N, 1./N, weight, 
                        ones_.cpu_data(), 0., temp1_.mutable_cpu_data()); // Ex
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M, N, 1, 
                        Dtype(1.), temp1_.cpu_data(), ones_.cpu_data(), Dtype(0),
                        temp2_.mutable_cpu_data()); 
  caffe_sub(count, weight, temp2_.cpu_data(), 
            temp1_.mutable_cpu_data()); // X - Ex
  caffe_powx(count, weight, Dtype(2.), 
             temp2_.mutable_cpu_data()); // x^2
  caffe_sub(count, ones_.cpu_data(), temp2_.cpu_data(), 
            temp2_.mutable_cpu_data()); // 1 - x^2
  caffe_mul(count, temp1_.cpu_data(), temp2_.cpu_data(), 
            temp2_.mutable_cpu_data()); // (1 - x^2)(X - Ex)
  caffe_powx(count, temp1_.cpu_data(), Dtype(2.),
             temp1_.mutable_cpu_data()); // (x - Ex) ^ 2
  caffe_mul(count, temp1_.cpu_data(), weight,
            temp1_.mutable_cpu_data()); // x * (x - Ex)
  caffe_cpu_gemv<Dtype>(CblasNoTrans, M, N, 1./N, 
                        temp2_.cpu_data(), ones_.cpu_data(), 0., 
                        temp2_.mutable_cpu_diff()); // E((1-x^2)(x-Ex))
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M, N,
                        1, Dtype(1.), temp2_.cpu_diff(), ones_.cpu_data(), Dtype(0),
                        temp1_.mutable_cpu_diff());
  caffe_add(count, temp1_.cpu_diff(), temp1_.cpu_data(),
            temp1_.mutable_cpu_data());
  caffe_sub(count, temp1_.cpu_data(), temp2_.cpu_data(), temp1_.mutable_cpu_data());
  Dtype weight_decay = this->layer_param_.apl_param().layer_weight_decay();
  caffe_scal(count, weight_decay, temp1_.mutable_cpu_data());
}
 ...
 */
template <class Dtype>
void APLLayer<Dtype>::L2_regularise(const int count, const Dtype* weight){
  Dtype weight_decay = this->layer_param_.apl_param().layer_weight_decay();
  caffe_cpu_axpby(count, weight_decay, weight, Dtype(0.), 
                  temp1_.mutable_cpu_data());
}
