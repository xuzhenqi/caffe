...
namespace caffe {
  ...
  // put the squares of bottom into buffer_blob_
    ...
    caffe_gpu_axpby(batch_mean_.count(), decay_, batch_mean_.gpu_data(), Dtype(1) - decay_, this->blobs_[2]->mutable_gpu_data());
    ...
    caffe_gpu_axpby(batch_variance_.count(), decay_, batch_variance_.gpu_data(), Dtype(1) - decay_, this->blobs_[3]->mutable_gpu_data());
        if (this->phase_==TEST && moving_average_) {
