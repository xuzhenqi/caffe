...
namespace caffe {
  ...
  void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    if (this->blobs_.size() > 0) {
      ...
      caffe_set(C_, Dtype(0), this->blobs_[2]->mutable_cpu_data());

      // history variance
      this->blobs_[3].reset(new Blob<Dtype>(1, C_, 1, 1));
      caffe_set(C_, Dtype(0), this->blobs_[3]->mutable_cpu_data());
  ...
  void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    ...
    caffe_cpu_axpby(batch_mean_.count(), decay_, batch_mean_.cpu_data(), Dtype(1) - decay_, this->blobs_[2]->mutable_cpu_data());
    ...
    caffe_cpu_axpby(batch_variance_.count(), decay_, batch_variance_.cpu_data(), Dtype(1) - decay_, this->blobs_[3]->mutable_cpu_data());
    if (this->phase_==TEST && moving_average_) {
