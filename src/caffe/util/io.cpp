...
bool ReadImagesToDatum(const vector<string>& filenames, Datum* datum,
                      const int height, const int width, const bool is_color) {
  if (filenames.size() == 0) {
    LOG(ERROR) << "filenames.size == 0";
    return false;
  }
  cv::Mat cv_img = ReadImageToCVMat(filenames[0], height, width, is_color);
  datum->set_channels(cv_img.channels() * filenames.size());
  datum->set_width(cv_img.cols);
  datum->set_height(cv_img.rows);
  std::string buffer(datum->channels() * datum->width() * datum->height(), ' ');
  for (int i = 0; i < filenames.size(); ++i) {
    if (i != 0)
      cv_img = ReadImageToCVMat(filenames[i], height, width, is_color);
    CHECK(cv_img.data) << "Cannot read image: " << filenames[i];
    CHECK_EQ(cv_img.rows, datum->height());
    CHECK_EQ(cv_img.cols, datum->width());
    for (int h = 0; h < datum->height(); ++h) {
      const uchar* ptr = cv_img.ptr<uchar>(h);
      int img_index = 0;
      for (int w = 0; w < datum->width(); ++w) {
        for (int c = 0; c < cv_img.channels(); ++c) {
          int datum_index = ((i*cv_img.channels() + c)* datum->height() + h) * datum->width() + w;
