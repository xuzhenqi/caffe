...
/**
 * @brief Compute sum operation on the input blobs, the last blob is
 * begin_marker.
 */
template <typename Dtype>
class SumRNNLayer : public Layer<Dtype> {
 public:
  explicit SumRNNLayer(const LayerParameter& param) : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                          const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
                       const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "SumRNN"; }
  // The last is begin_marker
  virtual inline int ExactNumBottomBlobs() const { return 3; }
  virtual inline int ExactNumTopBlobs() const { return 1; }

  inline const vector<Dtype>& GetCoeffs() {return coeffs_; }
...
template <typename Dtype>
class InnerProductRNNLayer : public Layer<Dtype> {
 public:
  explicit InnerProductRNNLayer(const LayerParameter& param)
      : Layer<Dtype>(param), neuron_layer_(new ReLULayer<Dtype>(param)) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
...
/**
 * @brief Takes a Blob and partition it according to params
 *        outputting multiple partitioned Blob results.
 *
 */
template <typename Dtype>
class PartitionLayer : public Layer<Dtype> {
 public:
  explicit PartitionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "Partition"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 1; }
...
/**
 * @brief Takes multiple Blob and combine them according to params
 *        outputting one Blob.
 *
 */
template <typename Dtype>
