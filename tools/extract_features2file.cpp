...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  shared_ptr<Net<Dtype> > feature_extraction_net(
      new Net<Dtype>(feature_extraction_proto, TEST));
  LOG(ERROR) << "Init Net";
  feature_extraction_net->CopyTrainedLayersFrom(pretrained_binary_proto);
  LOG(ERROR) << "Copy TrainParam";
  ...
  vector< FILE* >  feature_files ;
  for (size_t i = 0; i < num_features; ++i)
  {
	  leveldb_names[i].append(".fea");
	  LOG(INFO)<<"Opening file"<<leveldb_names[i];
	  FILE * file = std::fopen(leveldb_names[i].c_str(),"wb");
	  CHECK(file!=NULL) << "Failed to open file " << leveldb_names[i];
  ...
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < num_features; ++i) {
      ...
      int batch_size = feature_blob->num();//batch_size就是一次迭代的样本的数目
      int dim_features = feature_blob->count() / batch_size;
      if(batch_index==0)
      {
    	  int datanum=batch_size*num_mini_batches;
    	  std::fwrite((const void*)(&dim_features),sizeof(int),1,feature_files[i]);
    	  std::fwrite((const void*)(&datanum) ,sizeof(int),1,feature_files[i]);
      }
      ...
      for (int n = 0; n < batch_size; ++n) {
        ...
        /*my code start->将数据写入对应的特征文件*/
        std::fwrite(feature_blob_data,sizeof(Dtype),dim_features,feature_files[i]);
        /*my code end*/
        ++image_indices[i];
